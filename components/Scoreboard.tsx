  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-3 text-xs light:border-zinc-200 light:bg-zinc-100/50">
      ...
      <div className="grid grid-cols-3 gap-y-1 mb-3">
        <div>
          <div className="text-blue-400 font-mono text-lg font-bold leading-none">{score.phone_qsos}</div>
          <div className="text-zinc-500 text-[10px]">Phone</div>
        </div>
        <div>
          <div className="text-yellow-400 font-mono text-lg font-bold leading-none">{score.cw_qsos}</div>
          <div className="text-zinc-500 text-[10px]">CW</div>
        </div>
        <div>
          <div className="text-green-400 font-mono text-lg font-bold leading-none">{score.digital_qsos}</div>
        ...
        <div className="mt-3 border-t border-zinc-800 pt-2">
          ...
          {(Object.entries(score.by_band) as [string, { ph: number; cw: number; dig: number }][]).map(([band, stats]) => (
            <div key={band} className="flex items-center justify-between py-0.5">
              <span className="font-mono text-zinc-400">{band}</span>
              <span className="font-mono text-zinc-300">
                <span className="text-blue-400">{stats.ph}</span>/
                <span className="text-yellow-400">{stats.cw}</span>/
                <span className="text-green-400">{stats.dig}</span>
              </span>
            </div>
          ))}
        </div>
