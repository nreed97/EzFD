...
const EMPTY = { sent_class: null, section: null, name: null, user_text: null, known_master: false };
...
export async function GET(request: Request) {
  ...
  // Prefill is a convenience — degrade to "no match" rather than erroring.
  // Notably, a malformed event_id makes Postgres reject the uuid cast, which
  ...
  try {
    ...
    const row = rows[0];
    if (!row) return NextResponse.json({ callsign: upper, ...EMPTY });
    ...
    console.error('Call history lookup error:', err);
    return NextResponse.json({ callsign: upper, ...EMPTY });
  }
}
