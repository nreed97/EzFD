...
- **Run a whole event on a Pi with no internet** `Setup` — A machine you carry
  to the site can be the central log for the weekend with no uplink at all.
  Operators join over local WiFi and everything behaves normally, because from
  the app's point of view nothing is offline — dupes, band coordination and the
  live score are all server-side and the server is at the site. The install is
  the same `deploy.sh` as any other server, run in a different order: run it at
  home while you still have a network, leave the domain blank so it skips
  Let's Encrypt, and nothing it needs afterwards is on the internet. A Pi 4 or
  5 builds the app itself; below 2 GB of RAM it leans on the swap file
  `deploy.sh` adds and takes longer, which is a one-time cost paid at home. A
  new guide covers the parts that only bite offline — the clock, plain HTTP,
  giving the machine a name, power cuts, and merging the log back
  afterwards. ([#95])
  Docs: [Offline field servers → Set it up at home](field-server.md#set-it-up-at-home), [Offline field servers → What the machine has to be](field-server.md#what-the-machine-has-to-be)
