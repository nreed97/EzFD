...
Entries that change something you can see are labelled **[Scoring]** when a
claimed score moves, **[Exports]** when a Cabrillo, ADIF, summary sheet or
backup file changes, and **[Display]** when an operator sees or does something
different. Unlabelled entries are internal — worth knowing if you work on the
code, safe to skip if you just run events.
...
- **[Scoring, Display]** Winter Field Day is scored by its own rules ([#60]) — WFD uses
...
- **[Display]** Winter Field Day mobile stations log without a false warning ([#60]) —
...
- **[Display]** `MX` is accepted as a Winter Field Day exchange ([#60]) — Mexican
  stations send it, and it was being counted as a probable typo.
- **[Scoring, Exports]** The Field Day bonus schedule matches the official rules ([#59]) —
  ...
  on a large log.
- **[Scoring]** There is no Worked All Sections bonus ([#59]) — Field Day has never had
...
- **[Display]** The admin console pages its event list and reports failures honestly
...
- **[Display]** Band and mode coordination works for contests ([#47]) — a station can
  claim a band and mode, and another is warned rather than silently doubling
  up.
- **[Display]** A new event defaults to section MN ([#55]) — matching the example
  callsign, instead of an unrelated section.
- **[Display]** There is an operator quick start ([#56]) — for someone who has a join
  code and a radio and nothing to administer.

## 2026-08-22

- **[Exports, Display]** Deleted contacts are recoverable and leave an audit trail ([#45]) —
...
- **[Scoring, Exports]** Cabrillo carries the correct claimed score and transmitter numbers
...
- **[Scoring]** Only real sections count toward the sections total ([#36]) — a handful
  of typos could otherwise read as a clean sweep; `DX` is accepted as a valid
  exchange without being counted as a section.
- **[Scoring]** The ARRL/RAC section list is current ([#33]) — RAC's recent splits and
  renames were missing, so the sections-worked denominator was wrong.
- **[Display]** Server clock skew is detected and shown ([#33]) — a field server with
...
- **[Display]** Special event station support — one callsign across many operators,
  with band and mode checkout enforced by the database rather than by
  convention.
- **[Display]** Full documentation under `docs/`, with screenshots ([#25], [#26]).
- **[Display]** 24-hour clock pickers and a schedule timeline ([#27], [#28], [#29]).
