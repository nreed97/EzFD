...
| `scripts/test-sections.cjs` | The ARRL/RAC section list — the three enumerations agree, no hardcoded totals, and the map's boundary file covers every section exactly once |
...
- **The map's tiles must need no account.** `MapView` draws from `tile.openstreetmap.org`, which is keyless. It used to use CARTO's basemap CDN, which was open and then wasn't — and the refusal arrives as *"API key required" rendered into the tile image*, so the map still drew, still placed all 85 sections correctly, and reported nothing: no console error, no failed request, nothing on screen to say why it was unreadable. A key is the wrong shape here whatever it costs: there is no account to attach one to, the field servers this supports run on plain HTTP with no internet guarantee, and cloning the repo has to give you a working map. OSM publishes one style and it is light, so dark mode is a CSS filter scoped to `.leaflet-tile-pane` — never the whole container, which would invert the amber worked-section labels into an unreadable blue. `scripts/test-nav.cjs` fails if a keyed provider or an `api_key`/`access_token` reappears in the URL, and if the filter escapes the tile pane.
- **On the map, the fill answers "worked?" and the border answers "where does this section end?" — never the same colour doing both.** They shared amber once: a `#b45309` stroke under a `#fbbf24` fill at `fillOpacity: 0.5`. The fill is translucent over a basemap whose lightness *inverts* between themes, because dark mode is a filter on `.leaflet-tile-pane` and the overlay is not filtered — so in dark mode the fill blended toward the dark tiles and landed on the border's own luminance at **1.02:1**, the same lightness. Light mode measured 3.59:1, which is why it only looked broken in the theme that is the default, and why adjacent worked sections read as one blob. Every border is now a neutral chosen against the *page*, flipping with the theme as the unworked one always did; **no constant works**, because near-black is 4.05 on dark worked but 1.17 on dark unworked, near-white is the mirror, a darker amber is 1.85, and any mid-grey scores the same 1.02 the amber did. Two tiers, not one: a single strong neutral made the *empty* half of the map the loudest thing on it, which is the glare the night-adaptation note above exists to avoid. Order is load-bearing three times and every failure is silent — Leaflet draws all 85 polygons into one SVG group in document order and centres a stroke on its path, so fills must come before borders (otherwise half of each border is washed by a neighbour's 50% amber, which alone took light mode to 2.00:1), and the dim tier before the strong one (otherwise the shade of a shared worked/unworked edge depends on which polygon the file lists last). `scripts/test-nav.cjs` fails if `#b45309` returns, if the border stops flipping with the theme, or if any of that ordering changes. Do **not** reach for a colour per section: that puts *identity* in the fill, which is why such maps then need 85 hues and why the fill can no longer mean anything else.
