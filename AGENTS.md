...
| `scripts/test-sections.cjs` | The ARRL/RAC section list — the three enumerations agree, no hardcoded totals, the map's boundary file covers every section exactly once, and the basemap under it draws as a map rather than a smear |
...
| `scripts/test-clock.cjs` | What is holding the server's clock and what the devices make of it — an unknown never reads as a warning, and one device never convicts the server |
| `scripts/test-deploy-detect.sh` | Which machines `deploy.sh` will install on — Debian derivatives via `ID_LIKE`, a claimed heritage checked against whether apt is really there, and that anything else is refused fatally, before the operator is asked anything, with no second path below |
| `scripts/test-map-labels.cjs` | Which section labels the map draws at which zoom — nothing drawn overlaps anything else, placement never reads whether a section is worked, and zooming in never takes a label away |
...
- **An SSE stream never finishes, so shutdown has to end it.** Next's `SIGTERM` handler calls `server.close()` and waits for every connection, and `/api/realtime` streams are open for as long as a tab is — so one browser left on the dashboard held every `systemctl restart ezfd` until systemd SIGKILLed at 90s, which read as "Configuring systemd service" hanging on each deploy. The route keeps its open streams in a set on `globalThis` and ends them on `SIGTERM`/`SIGINT`; measured, exit goes from never to ~105 ms with streams open. Any new long-lived response (another stream, a long poll) must register the same way. `TimeoutStopSec=15` in the unit is only the backstop.
...
- **`public/sections.geo.json` is a fourth enumeration of the section list, and it is generated, never edited.** `scripts/build-section-geo.mjs` builds it from `us-atlas` (Census states and counties, npm, public domain) and Statistics Canada's census division cartographic boundary file for Canada, both pinned. 54 sections *are* an administrative unit or a small union of them — Maryland + DC, Yukon + NWT + Nunavut, Hawaii + the US Pacific islands. Another 27 are dissolved from counties using `data/arrl-section-counties.json`, transcribed from ARRL's published boundaries; that file keeps ARRL's own strings so it diffs against their page, and three differences are reconciled in the build rather than edited into it (ARRL spells Northampton PA "Northhampton", still calls Miami-Dade "Dade", and drops a comma so Mercer and Ocean NJ read as one county). The build verifies the table **both ways**: a name no county answers to fails, and so does a county in a carved state that no section claims — the second is the one that leaves a hole in the map, and a map with a hole in it looks like a map. Ontario's GH/ONE/ONN/ONS are RAC sections, absent from ARRL's document, and are dissolved from `data/rac-ontario-sections.json` — RAC's own table, transcribed the same way, with two reconciliations in the build (RAC lists Brant and Brantford, and Haldimand and Norfolk, as four rows where the census has two divisions) and one inference (RAC's table names the City of Greater Sudbury but not Sudbury District, which would leave a hole the size of Belgium; ARRL's section-boundaries page puts Killarney in ONN, and Killarney is in Sudbury District). `scripts/test-sections.cjs` checks the built file from outside — every section once, no `DX`, no ring spanning more than 180° (Alaska and PAC really do straddle the antimeridian and only render correctly because each island is its own ring), and under 400 KB. **That 180° rule does not transfer to `public/basemap.geo.json`**, where Afro-Eurasia is one connected ring genuinely 208° wide and no splitting makes it narrower; the basemap is asserted on the symptom instead — no step over 180° between consecutive vertices (the date-line jump itself, which draws as a band across the world), and no longitude outside ±200°. The second exists because unwrapping a ring across the date line carries a -360 offset that has to be taken back out: Afro-Eurasia's ring starts in Chukotka, so without the re-centring Europe and Africa were written at around -350° and Leaflet drew them a whole world-width to the left, leaving an empty Eastern Hemisphere and nothing on screen to say why.
- **Collecting rings is not dissolving.** A section built by concatenating its census divisions' rings into one MultiPolygon keeps every internal border, and Leaflet strokes every ring — the Golden Horseshoe drew as its seven constituent municipalities with the section boundary nowhere distinguishable among them, and the file was 96 KB bigger for the duplicated arcs. `tc.merge()` over a TopoJSON topology is what removes them, which is why Canada builds its own topology before the merge instead of relying on the `topology()` call at the end: that one shares arcs between features, it does not dissolve within one. The US counties always went through `tc.merge`; this was the Canadian path missing it.
- **Every Canadian section comes from the one Statistics Canada file, provinces included, and that is not an accident.** Ontario's four have to come from census divisions because that is how RAC carves them. The provinces moved there too because when Ontario's *neighbours* still came from Natural Earth the two outlines did not coincide, and a strip belonging to neither opened along the border — 1.9% of the Ottawa River border and 4.8% of the Manitoba one, measured by rendering the file over a red background and counting what showed through, not by looking at it. One source per country means every internal border is the same arcs on both sides and `topology()` welds them. The download is pinned by commit **and** by SHA-256: a pinned URL only says where the bytes came from, and a boundary file that changed underneath would redraw sections silently.
- **`SECTION_GEO_TOLERANCE` is an absolute simplification threshold, and it must not go back to being a percentile.** "Keep the most significant tenth of the vertices" measures the file, not the map, so adding a denser source makes the cut-off finer for *everything else*: adding Ontario's census divisions took the arc count from 22,930 to 392,019, dropped the threshold from 0.049 to 0.0000054, and tripled the Northern Territories' size without anything about the Northern Territories changing. The shipped value is the threshold the percentile produced when the file held only the original sources, so shapes calibrated against rendered output keep the detail they were calibrated to. Below about 0.02 the file stops shrinking usefully; above about 0.15 Michigan's Upper Peninsula and the Delmarva start to mangle.
- **One `pending` outline is not waiting on a transcription.** Nipissing District is split between ONE and ONN by Algonquin Park, which is not a census-division boundary, so it cannot be derived from the source every other Canadian shape comes from. It ships as a dashed outline naming both sections, and `scripts/test-sections.cjs` tells that case apart from a jurisdiction awaiting its county list: a jurisdiction *substitutes* for its sections, while a shared unit sits on top of sections drawn all around it, so counting the names it lists would report every one of them as appearing twice. A pending feature whose sections are all filled elsewhere is the shared kind.
- **`deploy.sh` supports Debian, Ubuntu and Raspberry Pi OS, and stops on anything else — deliberately, and it is not an oversight to be fixed.** Everything it does after the pre-flight assumes Debian's layout: that nginx reads server blocks from `sites-enabled`, that the PostgreSQL package creates and starts a cluster, that the firewall is `ufw`, that `nologin` is in `/usr/sbin`. Those were assumed silently on machines that did not hold them — a server block written where nginx never looks still passes `nginx -t`, because a file nobody includes is not a syntax error, so the deploy reported success and the site served the stock welcome page. The fix considered and rejected was to derive each fact per distribution: roughly 240 lines that cannot be tested here against a real Fedora, Arch or SUSE, and a script quietly wrong on four distributions is worse than one loudly narrow on three. The app itself is an ordinary Node standalone build against PostgreSQL and runs anywhere by hand; `docs/deployment.md` gives the shape without claiming it is tested. `scripts/test-deploy-detect.sh` fails if the refusal stops being fatal, if it moves after the prompts, or if a second non-apt path reappears below it. Widen this only when there is a way to prove the wider path works end to end — which a container cannot do for the property a field server most needs, coming back after a power cut.
- **There is one way to install EzFD and it is `deploy.sh`; the network is an *install-time* dependency, not a run-time one.** A second deployment path aimed at offline field servers was built and removed before it shipped, and the reasoning is why there is still only one: `deploy.sh` already prompts for a blank domain (IP-only, no certbot), and its entire apt/PGDG block is gated behind a fresh install (Node alone is also checked on an update, and upgraded only when below `.nvmrc`), so the only thing on a no-domain run that touches the network is `npm ci` and `next/font/google`. Any alternative install carries that same constraint, so it buys nothing and costs the usual drift. Install at home, carry the machine to the site. Do **not** reintroduce "the Pi is too small to build": a cold `next build` peaks around 550 MB across the Node process tree and completes with the JS heap capped at `--max-old-space-size=256`, and the swap file `deploy.sh` adds below 2 GB of RAM is aimed at 1 GB droplets, which build on every deploy anyway. The real variable on a Pi is storage, not RAM — `npm ci` unpacks ~22,000 files, which is what a cheap microSD is worst at.
- **The Node major lives in `.nvmrc` and nowhere else.** CI's `setup-node` reads it with `node-version-file`, `deploy.sh` installs or upgrades to it, and `ezfd-admin.sh`'s update warns below it. `deploy.sh` used to hardcode `setup_20.x` inside the fresh-install block while CI built on 22, so every deployed server ran a version CI never tested, and an update never moved it — servers sat on Node 20 past its end of life. It is a floor: a newer Node is never downgraded. NodeSource publishes Node 20 and later for amd64 and arm64 only, so a 32-bit Raspberry Pi OS install has never been able to run `deploy.sh`; that is documented in `docs/field-server.md`, not worked around.
- **Ask how long since the clock was disciplined, never whether NTP is synchronised right now.** On an offline field server `NTPSynchronized` is `no` permanently and by design, so a warning built on it fires hardest at the operator who fitted an RTC or a GPS and did everything right — `ezfd-admin.sh` already had to be fixed for exactly this, and `ClockSkewBanner` would have repeated it. `lib/clockSource.ts` reports a *last disciplined* time instead, which reads correctly on GPS, on an RTC and on a bare machine alike, because the age of that event is the bound on the error. The rule that falls out of it and that `scripts/test-clock.cjs` enforces: **an unknown is not a warning.** Every probe degrades to `null`, and both `clockIsStale()` and `clockIsUnaccountedFor()` require *positive evidence of absence* — a container with no `timedatectl` answers nothing at all, and reporting that as "nothing is holding this clock" is the same false alarm in new clothes. That one was caught by running the real endpoint, not by reasoning: it reported `unaccounted_for` on a machine that had simply never been asked.
- **One device disagreeing with the server is a disagreement; three agreeing is a verdict.** `lib/clockQuorum.ts` exists because the banner could only ever say "these two clocks differ" and deliberately would not say which was wrong — honest, and useless to the operator who has to decide what to fix. A dozen phones, mostly carrier-synchronised, are collectively a far better authority than a field server asking itself. The thresholds are not decoration: **three** distinct devices minimum, because with two a single wrong laptop is half the sample; a **two-thirds** majority **in one direction**, because devices split either side of the server are two operators' own clocks being wrong, not a server fault; and **only the newest observation per device**, or one browser left open overnight polling every five minutes outvotes the whole field. Below the bar the banner keeps its old both-ways wording — a confident wrong verdict is worse than an honest hedge. Observations are in-memory and TTL'd at fifteen minutes on purpose: a reading from an hour ago may predate the admin console fixing the clock, and persisting it would buy nothing and cost a migration. That relies on EzFD being one `node server.js`; if it ever runs as several processes this must move rather than be quietly wrong.
- **`fake-hwclock` is not a clock and must never be counted as a time source.** It writes the time to a file at shutdown and restores it at boot, so a Pi carrying it comes up looking confident and wrong by however long it was switched off — the precise failure #17 exists to surface. `detect_clock_sources()` in `ezfd-admin.sh` reports it by name and excludes it from the verdict. The same function exists because the screen used to ask `timedatectl` one question (`NTPSynchronized`) and tell anyone who answered "no" to fit an RTC module — advice aimed squarely at the operator most likely to have already fitted one, since an offline field server with a working RTC answers "no" by definition. A warning that fires hardest at the person who did the right thing is how warnings get ignored.
- **The map fetches nothing from anybody, and a raster tile layer must not come back.** `MapView` draws its own ground from `public/basemap.geo.json` — Natural Earth 110m land, built by `scripts/build-basemap.mjs` from the pinned `world-atlas` package, 56 KB, one feature, one SVG path. It drew CARTO's tiles first, then OpenStreetMap's, and three separate problems pointed at the same answer. CARTO was open and then wasn't, and the refusal arrived as *"API key required" rendered into the tile image* — the map still drew, still placed all 85 sections correctly, and reported nothing: no console error, no failed request, nothing on screen to say why it was unreadable. OSM removed the key but not the dependency: theirs is a volunteer service with a usage policy, and EzFD is cloned and deployed by whoever wants it, so every install pointed at their servers. **Firefox** settled it — measured on a running event, panning with tiles dropped 11 of 319 frames at a p95 of 30 ms; with no tiles and everything else identical, 0 of 517 at 6.1 ms, and neither the renderer (SVG and Canvas measured the same), nor Leaflet's tile options, nor forcing the tile pane onto its own compositor layer moved it. And on an offline field server the tiles never loaded at all, which is the deployment this app exists for. Dark mode is now a colour rather than a CSS filter over the tile pane, so `.map-dark` and the `invert(1) hue-rotate(180deg)` chain are both gone. **`#f2efe9` and `#1c1a16` are not free choices** — they are what the old raster basemap *rendered as* underneath the section fills, and the worked/unworked border contrast below was calibrated against exactly those values. `scripts/test-nav.cjs` fails if a `<TileLayer>`, an `L.tileLayer(` or any third-party tile host reappears, if `basemap.geo.json` goes missing or over 120 KB, or if either land colour changes.
- **On the map, the fill answers "worked?" and the border answers "where does this section end?" — never the same colour doing both.** They shared amber once: a `#b45309` stroke under a `#fbbf24` fill at `fillOpacity: 0.5`. The fill is translucent over a basemap whose lightness *inverts* between themes — so in dark mode the fill blended toward the dark ground and landed on the border's own luminance at **1.02:1**, the same lightness. Light mode measured 3.59:1, which is why it only looked broken in the theme that is the default, and why adjacent worked sections read as one blob. Every border is now a neutral chosen against the *page*, flipping with the theme as the unworked one always did; **no constant works**, because near-black is 4.05 on dark worked but 1.17 on dark unworked, near-white is the mirror, a darker amber is 1.85, and any mid-grey scores the same 1.02 the amber did. Two tiers, not one: a single strong neutral made the *empty* half of the map the loudest thing on it, which is the glare the night-adaptation note above exists to avoid. Order is load-bearing three times and every failure is silent — Leaflet draws all 85 polygons into one SVG group in document order and centres a stroke on its path, so fills must come before borders (otherwise half of each border is washed by a neighbour's 50% amber, which alone took light mode to 2.00:1), and the dim tier before the strong one (otherwise the shade of a shared worked/unworked edge depends on which polygon the file lists last). `scripts/test-nav.cjs` fails if `#b45309` returns, if the border stops flipping with the theme, or if any of that ordering changes. Do **not** reach for a colour per section: that puts *identity* in the fill, which is why such maps then need 85 hues and why the fill can no longer mean anything else.
- **The map's labels are placed, not just drawn, and placement must be blind to the log and monotonic in zoom.** At the default zoom of 3, **54 of the 85 labels overlapped another** — 105 pairs, worst through the Northeast where a dozen sections are smaller than their own abbreviations. `lib/mapLabels.ts` keeps a label only when its box clears every box already kept, which takes zoom 3 to 50 drawn and none overlapping; by zoom 6 it is 84 of 85. Leader lines were the alternative and are what a printed atlas would do, but they need a placement solver and at zoom 3 would mean fanning fifteen labels into the Atlantic without crossing. Two properties carry more weight than optimal packing, and neither is visible in a screenshot. **Placement never consults `worked`** — a label that vanished because somebody logged a contact would make the map twitch mid-run, so the module has no notion of it and `scripts/test-map-labels.cjs` greps the compiled-away-comments source to keep it that way (as a substring, since `\bworked\b` matches neither `isWorked` nor `workedFirst`). **Zooming in never removes a label**, which does *not* fall out of greedy placement and was wrong in the first cut: a label blocked at one zoom becomes placeable at the next and can then evict one that had been drawn all along — measured, VT and DE disappeared between zoom 3 and 4, ENY between 4 and 5, so zooming in to read a label made it vanish. Each level therefore starts from the previous level's set already placed and only fills the gaps around it, and fractional zooms floor so nothing flickers mid-pinch. Hiding a label costs nothing only because the section polygon carries the same tooltip; before that, a shape answered nothing and the only way to identify a section was to hit a 9px label box. The label colours had the same inversion as everything else on this map: unworked text was `#52525b` on a near-black box at **2.32:1** while worked sat at 10.48:1, so the labels an operator is hunting were the unreadable ones, in the default theme only.
