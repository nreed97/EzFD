...
- **`public/sections.geo.json` is a fourth enumeration of the section list, and it is generated, never edited.** `scripts/build-section-geo.mjs` builds it from `us-atlas` (Census states and counties, npm, public domain) and Natural Earth 1:50m admin-1 for Canada, both pinned. 54 sections *are* an administrative unit or a small union of them — Maryland + DC, Yukon + NWT + Nunavut, Hawaii + the US Pacific islands. Another 27 are dissolved from counties using `data/arrl-section-counties.json`, transcribed from ARRL's published boundaries; that file keeps ARRL's own strings so it diffs against their page, and three differences are reconciled in the build rather than edited into it (ARRL spells Northampton PA "Northhampton", still calls Miami-Dade "Dade", and drops a comma so Mercer and Ocean NJ read as one county). The build verifies the table **both ways**: a name no county answers to fails, and so does a county in a carved state that no section claims — the second is the one that leaves a hole in the map, and a map with a hole in it looks like a map. Ontario's GH/ONE/ONN/ONS are RAC sections, absent from ARRL's document, so they ship as one dashed `pending` outline; drawing a guessed boundary would be indistinguishable from a real one. `scripts/test-sections.cjs` checks the built file from outside — every section once, no `DX`, no ring spanning more than 180° (Alaska and PAC really do straddle the antimeridian and only render correctly because each island is its own ring), and under 400 KB. Simplification was calibrated against rendered output: below `SECTION_GEO_KEEP=0.05` Michigan's Upper Peninsula and the Delmarva start to mangle.
