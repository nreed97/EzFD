...
| `scripts/test-compose.sh` | The field-server container stack, run for real — schema applied before the app starts, the log surviving a hard kill *and* a container replacement, db-init idempotent |
...
- **A power cut and an upgrade are different tests, and only one of them needs the volume.** `docker compose kill` then `up` restarts the *same* containers, so the log survives it even with no volume configured — the `postgres` image declares a `VOLUME` and Docker supplies an anonymous one. Recreating the containers is what orphans that anonymous volume, and recreating them is exactly what an ordinary upgrade between events does. A `compose.yaml` missing `db-data:` therefore looks correct through every power cut and loses the club's log the first time someone updates the app. `scripts/test-compose.sh` asserts both, and the down/up one is the load-bearing half: removing the named volume leaves the kill/restart check green.
- **The container stack provisions the database the same three ways `deploy.sh` and CI do, and must not grow a fourth.** `docker/db-init.sh` creates the `ezfd` role, creates the database, and applies `db/schema.sql` as the superuser — no container-specific SQL, no migration list of its own. That is safe precisely because CI applies `schema.sql` twice against an empty database and then runs the full suite, which is what makes it the one true provisioning step. It runs as a one-shot service on every `up`, and the app carries `condition: service_completed_successfully` on it, so a half-applied schema stops the app from starting rather than being discovered mid-event. `ON_ERROR_STOP=1` is load-bearing there for the usual reason: psql fed a file exits 0 even when a statement failed.
- **`HOSTNAME=0.0.0.0` in the Dockerfile is not a copy of `deploy.sh`'s `127.0.0.1`, and swapping it breaks the container silently.** On a VPS the app sits behind nginx and binding loopback is what stops it being reachable except through it. In a container the namespace boundary already does that job, and binding loopback would publish a port with nothing listening on it — the container would look healthy from inside and refuse every connection from outside.
- **Building the image needs internet; running it does not — and the network is the *only* thing that blocks a build.** `npm ci` fetches the dependency tree and `next/font/google` fetches the two typefaces at build time, so `docker compose build` at the site is not a fallback, and that is why `docs/field-server.md` documents `docker save`/`docker load` rather than assuming a registry is reachable. Do **not** restate this as "the Pi is too small to build": a cold `next build` peaks around 550 MB across the Node process tree and completes with the JS heap capped at `--max-old-space-size=256`, and `deploy.sh` only adds swap below 2 GB of RAM — a threshold aimed at 1 GB droplets, which build the app on every deploy anyway. The guide said "Don't build on the Pi" and cited that swap line as evidence, which had it backwards: the threshold is what shows a 4-8 GB Pi 5 is clear of the problem. Cross-building arm64 on an x86 laptop runs under QEMU and is *slower* than building natively on a Pi 5, so `docker save` is about reach — machines with no registry access — never speed. The real Pi variable is storage, not RAM: `npm ci` unpacks ~22,000 files, which is what a cheap microSD is worst at.
