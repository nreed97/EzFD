...
- **Building the image needs internet; running it does not.** `npm ci` fetches the dependency tree and `next/font/google` fetches the two typefaces at build time. That is fine — you build at home — but it means `docker compose build` at the site is not a fallback, and it is why `docs/field-server.md` documents `docker save`/`docker load` rather than assuming a registry is reachable.
