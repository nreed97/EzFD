...
- **There is one way to install EzFD and it is `deploy.sh`; the network is an *install-time* dependency, not a run-time one.** A container stack briefly existed alongside it and was removed: `deploy.sh` already prompts for a blank domain (IP-only, no certbot), and its entire apt/NodeSource/PGDG block is gated behind a fresh install, so the only thing on a no-domain run that touches the network is `npm ci` and `next/font/google`. That is the same constraint the containers had, so the second deployment path was buying nothing and costing the usual drift. Install at home, carry the machine to the site. Do **not** reintroduce "the Pi is too small to build": a cold `next build` peaks around 550 MB across the Node process tree and completes with the JS heap capped at `--max-old-space-size=256`, and the swap file `deploy.sh` adds below 2 GB of RAM is aimed at 1 GB droplets, which build on every deploy anyway. The real variable on a Pi is storage, not RAM — `npm ci` unpacks ~22,000 files, which is what a cheap microSD is worst at.
